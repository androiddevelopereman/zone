...
const displayZonePolygon = () => {
    if (!zoneData.value || !zoneData.value.coordinates || zoneData.value.coordinates.length === 0) {
...
const saveChanges = async (geometry) => {
    ...
    try {
        ...
        saveError.value = `Error updating zone: ${err.response?.data?.message || err.message || 'Unknown error'}`
...
<template>
    <div class="container">
        ...
        <p v-if="saveSuccess" class="success-message">Changes saved successfully!</p>
